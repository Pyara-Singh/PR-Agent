...
from PR_Agent.schemas import AgentResult, PullRequestCreate
...
async def create_or_get_webhook_review(
    session: AsyncSession, pull_request: PullRequest, delivery_id: str
) -> tuple[ReviewSession, bool]:
    """Return the original review when GitHub retries the same delivery."""
    if delivery_id:
        existing = await session.scalar(
            select(ReviewSession).where(ReviewSession.source_delivery_id == delivery_id)
        )
        if existing is not None:
            return existing, False
    review = ReviewSession(
        pull_request_id=pull_request.id,
        source_delivery_id=delivery_id or None,
    )
    session.add(review)
    await session.commit()
    return review, True
